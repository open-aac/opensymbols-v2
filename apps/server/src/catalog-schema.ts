...
ALTER TABLE catalog_symbol_localizations
...
CREATE TABLE IF NOT EXISTS catalog_symbol_search_signals (
  ...
  symbol_id bigint NOT NULL REFERENCES catalog_symbols(id) ON DELETE CASCADE,
  locale text NOT NULL,
  scope text NOT NULL CHECK (scope IN ('base', 'localization')),
  ordinal integer NOT NULL,
  term text NOT NULL,
  signal_type text NOT NULL CHECK (signal_type IN ('boost', 'use_score')),
  score double precision NOT NULL,
  PRIMARY KEY (symbol_id, locale, scope, term, signal_type)
);

ALTER TABLE catalog_symbol_search_signals
  ADD COLUMN IF NOT EXISTS scope text NOT NULL DEFAULT 'localization';
ALTER TABLE catalog_symbol_search_signals
  ADD COLUMN IF NOT EXISTS ordinal integer NOT NULL DEFAULT 0;
ALTER TABLE catalog_symbol_search_signals
  DROP CONSTRAINT IF EXISTS catalog_symbol_search_signals_scope_check;
ALTER TABLE catalog_symbol_search_signals
  ADD CONSTRAINT catalog_symbol_search_signals_scope_check
  CHECK (scope IN ('base', 'localization'));
ALTER TABLE catalog_symbol_search_signals
  DROP CONSTRAINT IF EXISTS catalog_symbol_search_signals_pkey;
ALTER TABLE catalog_symbol_search_signals
  ADD PRIMARY KEY (symbol_id, locale, scope, term, signal_type);
ALTER TABLE catalog_symbol_search_signals
...
CREATE TABLE IF NOT EXISTS catalog_symbol_requests (
  ...
  migration_run_id uuid REFERENCES catalog_migration_runs(id) ON DELETE CASCADE,
...
DROP INDEX IF EXISTS index_catalog_requests_on_locale_phrase;
CREATE UNIQUE INDEX index_catalog_requests_on_locale_phrase ON catalog_symbol_requests(locale, phrase);
...
ALTER TABLE catalog_symbol_requests ALTER COLUMN migration_run_id DROP NOT NULL;
ALTER TABLE catalog_symbol_request_comments ALTER COLUMN migration_run_id DROP NOT NULL;
ALTER TABLE catalog_api_clients ALTER COLUMN migration_run_id DROP NOT NULL;
DO $$
BEGIN
  IF (SELECT is_identity = 'NO' FROM information_schema.columns
      WHERE table_schema = current_schema() AND table_name = 'catalog_symbol_requests' AND column_name = 'id') THEN
    ALTER TABLE catalog_symbol_requests ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;
  END IF;
  IF (SELECT is_identity = 'NO' FROM information_schema.columns
      WHERE table_schema = current_schema() AND table_name = 'catalog_api_clients' AND column_name = 'id') THEN
    ALTER TABLE catalog_api_clients ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;
  END IF;
END $$;
`
