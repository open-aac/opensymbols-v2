...
import { applyCharacterColours, hairColourOptions } from './character-template'
...
describe('character template colours', () => {
  it('updates every labelled skin and hair fill while preserving the artwork', () => {
  ...
  it('removes active content, event handlers, and external references', () => {
    ...
    const result = applyCharacterColours(unsafe.replace('</svg>', '<path id="haircolor-hair" style="fill:#000000" /></svg>'), {
      skinColour: '#ae724c',
      hairColour: '#17120f',
    })
    const updated = parse(result.svg)
  ...
  it('rejects malformed templates, missing labelled regions, and arbitrary colour values', () => {
    const colours = { skinColour: '#ae724c', hairColour: '#17120f' }
